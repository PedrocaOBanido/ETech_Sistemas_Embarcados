...
byte wave_type_sine = 0; // Sine
byte wave_type_triangle = 1; // Triangle
byte wave_type_sawtooth = 2; // Sawtooth
byte wave_type_quare = 3; // Square
byte wave_type;
void setup() {
  Serial.begin(115200);
}
...
void loop() {
  if (Serial.available()){
    Serial.println("solta o verbo...");
    char message = Serial.read();
    if (message == 'S'){
      wave_type = wave_type_sine;
    }else if(message == 'T'){
      wave_type = wave_type_triangle;
    }else if(message == 'D'){
      wave_type = wave_type_sawtooth;
    }else if(message == 'Q'){
      wave_type = wave_type_quare;
    }
  }
